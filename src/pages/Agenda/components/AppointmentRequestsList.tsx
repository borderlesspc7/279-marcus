...
import "../../Admin/AppointmentRequests.css";

interface AppointmentRequestsListProps {
  onApprove?: () => void;
}

export const AppointmentRequestsList: React.FC<AppointmentRequestsListProps> = ({ onApprove }) => {
  const { user } = useAuth();
  ...
  const handleApprove = async (appointmentId: string) => {
    try {
      ...
      // Chamar callback para recarregar agenda
      if (onApprove) {
        onApprove();
      }
    } catch (error) {
  ...
  return (
    <div className="appointment-requests appointment-requests--embedded">
      {requests.length === 0 ? (
