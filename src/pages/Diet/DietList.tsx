...
import { FaSpinner, FaUtensils, FaSearch } from "react-icons/fa";
...
interface DietWithClient extends Diet {
  client?: Client;
}
...
export const DietList: React.FC = () => {
  ...
  const { user } = useAuth();
  const [diets, setDiets] = useState<DietWithClient[]>([]);
  const [filteredDiets, setFilteredDiets] = useState<DietWithClient[]>([]);
  ...
  const loadDiets = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const dietsData = await getDietsByNutritionist(user.uid);
      
      // Carregar dados dos clientes
      const dietsWithClients = await Promise.all(
        dietsData.map(async (diet) => {
          try {
            const client = await getClientById(diet.clientId);
            return { ...diet, client: client || undefined };
          } catch (error) {
            console.error(`Erro ao carregar cliente ${diet.clientId}:`, error);
            return { ...diet, client: undefined };
          }
        })
      );

      setDiets(dietsWithClients);
    } catch (error) {
  ...
  const filterDiets = useCallback(() => {
    ...
    const filtered = diets.filter(
      (diet) =>
        ...
        diet.client?.fullName.toLowerCase().includes(query) ||
  ...
  const handleDietClick = (dietId: string) => {
    navigate(`/dashboard/dietas/${dietId}`);
  };
      ...
      <div className="diet-list__header">
        <div>
          ...
          <p className="diet-list__subtitle">
            Visualize e gerencie todas as dietas criadas
          </p>
        </div>
        <Button
          variant="primary"
          onClick={handleCreateNew}
          className="diet-list__add-button"
        >
          <FaUtensils /> Criar Nova Dieta
        </Button>
      </div>
        ...
        <div className="diet-list__empty">
          ...
          {!searchQuery && (
            <Button variant="primary" onClick={handleCreateNew}>
              <FaUtensils /> Criar Primeira Dieta
            </Button>
          )}
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                <div className="diet-card__footer">
                  ...
                  <span className="diet-card__arrow">→</span>
                </div>
              </div>
            );
          })}
